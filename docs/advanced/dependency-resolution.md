...
Wishtools 不仅仅比较字符串，而是将版本号解析为**语义元组序列**。
...
求解器将每个 `(Package, Version)` 对映射为一个唯一的整数 ID（布尔变量）。
...
硬约束是必须满足的条件，转换为 CNF 子句。如果无法满足，求解器返回 UNSAT（无解）。
...
当存在多个满足硬约束的解时，求解器通过最大化权重和来选择"最优"解。
...
**结论**:
系统会绝对优先满足 `python` 的版本需求（权重级 101w），其次是 `requests`（权重级 100w）。即使 `urllib3` 有一个新版本能极大增加总权重，只要它导致 `python` 或 `requests` 无法选到最新版（例如产生了冲突），求解器也会放弃它。
...
结果：**UNSATISFIABLE** (无解)。
SAT 求解器会迅速检测到这种逻辑矛盾，并报告冲突路径，而不是像某些包管理器那样陷入无限循环或随机选择一个版本。

### 4. 渐进式求解策略 (Progressive Solving)

为了提高性能，`Solver.collect_solution` 采用分阶段策略：
...
当使用 `wish +` (Verbose Mode) 时，如果求解失败，Solver 会输出详细的诊断信息。
...
1.  **算法复杂度**: 传统的贪心或回溯算法在面对复杂的依赖关系（如 "钻石依赖"）时，时间复杂度可能是指数级的。SAT 求解器能极其高效地处理成千上万个变量的约束。
